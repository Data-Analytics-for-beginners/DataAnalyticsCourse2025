...
# Резюме: Як я почав бачити світ як дані
...
![](https://miro.medium.com/v2/resize:fit:1100/format:webp/1*15b1Us8y4whAK5i2ajPviQ.jpeg)
...
# Резюме: Створення ШІ-агента для перетворення SQL баз даних у дашборди

- https://viveksinghpathania.medium.com/build-an-ai-agent-that-turns-sql-databases-into-dashboards-no-queries-needed-ea78571b2475
  

**Основна проблема:** Вузькі місця в доступі до даних - маркетинг подає заявки, продажі чекають місячних звітів, керівники не можуть отримати потрібні цифри швидко.

## Архітектура рішення

**Компоненти системи:**
- **Streamlit** - користувацький інтерфейс
- **LLM агенти** (Claude/GPT) - розуміння запитів і генерація SQL
- **MCP сервер** - безпечний шар з обмеженням до схеми та SELECT запитів
- **Існуюча SQL база даних** - без змін

## Технічна реалізація

### Безпечний MCP сервер
```python
class SqlReadOnlyServer:
    # Підключення лише для читання
    # Валідація - тільки SELECT запити
    # Отримання схеми бази даних
    # Виконання безпечних запитів
```

### Генератор дашбордів
Триступеневий pipeline:
1. **Аналіз схеми БД** - розуміння структури даних
2. **Отримання даних** - виконання SQL запитів на основі аналізу  
3. **Генерація HTML дашборду** - створення інтерактивних візуалізацій

### Чат-агент
Перетворює природномовні питання в SQL запити та повертає зрозумілі відповіді через MCP інструменти.

### Streamlit інтерфейс
Два таби:
- **Chatbot** - діалогове вікно для запитів до БД
- **Dashboard** - автоматично згенеровані візуалізації з можливістю завантаження

## Переваги системи

**Для бізнесу:**
- Швидші рішення без технічних бар'єрів
- Аналітики зосереджуються на складніших проблемах  
- Дані стають спільним активом, а не чорною скринькою

**Безпека:**
- Тільки читання з бази даних
- Обмеження на рівні схеми
- Валідація всіх запитів

## Практичне використання

1. Користувач пише питання природною мовою
2. ШІ аналізує схему БД та генерує SQL
3. Система виконує безпечний запит  
4. Результат повертається як відповідь або візуалізація
5. Можливість завантаження готових дашбордів

**Результат:** Система ліквідовує технічні бар'єри між користувачами та даними, забезпечуючи миттєвий доступ до інсайтів через природну мову при збереженні повної безпеки даних.
